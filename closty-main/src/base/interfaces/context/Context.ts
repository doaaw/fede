...
type BaseInteraction = AppInteraction | CommandInteraction | SelectMenuInteraction | ButtonInteraction | ModalSubmitInteraction;
  ...
  constructor () {
    ...
    this.menu = {
      ...
      async paginate (interaction: BaseInteraction, options: PaginateOptions): Promise<void> {
        ...
        async function components (status?: boolean): Promise<Array<ActionRow<MessageActionRowComponent>>> {

          if (!status) ++current.default;

          let components: Array<ActionRow<MessageActionRowComponent>> = [];
        ...
        let sortCollector: InteractionCollector<SelectMenuInteraction> = message.createMessageComponentCollector({ componentType: ComponentType.SelectMenu, filter: (select_menu: SelectMenuInteraction) => select_menu.customId === 'sort', time: 5 * 60 * 1000 });
        let listCollector: InteractionCollector<SelectMenuInteraction> = message.createMessageComponentCollector({ componentType: ComponentType.SelectMenu, filter: (select_menu: SelectMenuInteraction) => select_menu.customId === 'list', time: 5 * 60 * 1000 });
        ...
        sortCollector.on('collect', async (select_menu: SelectMenuInteraction) => {
        ...
        listCollector.on('collect', async (select_menu: SelectMenuInteraction) => {
      ...
      async paginateWithTransactions (interaction: BaseInteraction, options: PaginateWithTransactionsOptions): Promise<void> {
        ...
        let transaction: Array<{ emoji?: string; label?: string; description?: string; value: string; execute: (menu: SelectMenuInteraction, ...items: any[]) => Promise<void>; }> = transactions ? (await transactions.options(current.first, current.last, sorts ? sorts[current.sort].sort : undefined, filters ? filters[current.filter].filter : undefined)) : undefined;
        ...
        let components = async (status?: boolean): Promise<Array<ActionRow<MessageActionRowComponent>>> => {
              ...
              new ActionRow<MessageActionRowComponent>({
                components: [
                  new SelectMenu({
                    ...
                    disabled: status,
                    options: await Promise.all(transaction.map((select_menu: { emoji?: string; label?: string; description?: string; value: string; execute: (menu: SelectMenuInteraction, ...items: any[]) => Promise<void>; }) => {
                      return {
                        emoji: select_menu.emoji === undefined ? undefined : { id: select_menu.emoji },
                        label: select_menu.label || 'undefined',
                        description: select_menu.description,
        ...
        let sortCollector: InteractionCollector<SelectMenuInteraction> = message.createMessageComponentCollector({ componentType: ComponentType.SelectMenu, filter: (select_menu: SelectMenuInteraction) => select_menu.customId === 'sort', time: 5 * 60 * 1000 });
        let listCollector: InteractionCollector<SelectMenuInteraction> = message.createMessageComponentCollector({ componentType: ComponentType.SelectMenu, filter: (select_menu: SelectMenuInteraction) => select_menu.customId === 'list', time: 5 * 60 * 1000 });
        let transactionCollector: InteractionCollector<SelectMenuInteraction> = message.createMessageComponentCollector({ componentType: ComponentType.SelectMenu, filter: (select_menu: SelectMenuInteraction) => select_menu.customId === 'transaction', time: 5 * 60 * 1000 });
        let buttonCollector: InteractionCollector<ButtonInteraction> = message.createMessageComponentCollector({ componentType: ComponentType.Button, time: 5 * 60 * 1000 });
        ...
        transactionCollector.on('collect', async (select_menu: SelectMenuInteraction) => {
