...
export default class extends Category {
  ...
  async execute ({ interaction }: { interaction: CommandInteraction; }): Execute {
    ...
    async function components (): Promise<Array<ActionRow<MessageActionRowComponent>>> {
    ...
    async function Return (i: SelectMenuInteraction | ButtonInteraction, content: string | string[], edit?: boolean): Promise<void> {
    ...
    let listCollector: InteractionCollector<SelectMenuInteraction> = message.createMessageComponentCollector({ componentType: ComponentType.SelectMenu, filter: (select_menu: SelectMenuInteraction) => select_menu.customId === 'list' });
    ...
    listCollector.on('collect', async (select_menu: SelectMenuInteraction) => {
      ...
      let [ roleId ]: string[] = select_menu.values;
      let role: Role = interaction.guild.roles.cache.get(roleId);

      if (await Autorole.findOne({ guild: interaction.guild.id, roles: roleId })) {

        await select_menu.deferUpdate();

        await Autorole.findOneAndUpdate({ guild: interaction.guild.id }, { $pull: { roles: roleId } }, { upsert: true });
      ...
      await Autorole.findOneAndUpdate({ guild: interaction.guild.id }, { $push: { roles: roleId } }, { upsert: true });
